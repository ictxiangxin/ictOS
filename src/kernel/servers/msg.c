...
PUBLIC VOID recv_msg ( MSG* msg )
{
    ...
    {
        send_msg ( PID_KPM, KPM_WAITMSG, NULL, NULL );	/* tell the pm that I will wait msg */
...
{
    if ( kernelproclist.procs[dest_proc_id].state == KPS_SLEEP )    /* if the proc is sleep */
    {
    ...
    if ( kernelproclist.procs[dest_proc_id].state == KPS_WAITMSG )
